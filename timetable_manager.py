...
def get_timetable_for_class(lesson, year, delta_days=14):
	...
	if timetable_response.status_code != 200:
			print("Error while getting classes for year " + year + ", curriculum " + lesson['curriculum'] + " and class " + lesson['cod_modulo'] + " " + lesson['title'] + ". Url: " + timetable_response.url)
			return None
	...
	for lesson_event in timetable_response.json():
		...
		event.add('dtstart', datetime.strptime(lesson_event['start'], "%Y-%m-%dT%H:%M:%S").astimezone(pytz.UTC))
		event.add('dtend', datetime.strptime(lesson_event['end'], "%Y-%m-%dT%H:%M:%S").astimezone(pytz.UTC))
